  ...
  return (
    <div className="bg-white border border-border rounded-2xl p-3 flex flex-col sm:flex-row items-center gap-3 justify-between">
      <div className="flex items-center gap-2 text-xs font-semibold text-muted-foreground">
        ...
        <span>
          لكل صفحة · {from}–{to} من {total}
        </span>
      </div>

      <div className="flex items-center gap-1">
        <NavBtn disabled={current <= 1} onClick={() => onPageChange(1)} label="الأولى">
          <ChevronsRight className="size-4 rtl:hidden" />
          <ChevronsLeft className="size-4 ltr:hidden" />
        </NavBtn>
        <NavBtn disabled={current <= 1} onClick={() => onPageChange(current - 1)} label="السابق">
